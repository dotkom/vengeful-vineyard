...
export function sortGroupUsers(members: GroupUser[], punishments: PunishmentType[]) {
  const punishmentMap = new Map(punishments.map((p) => [p.punishment_type_id, p]))
  return members.sort((a, b) => {
    const count_punishments = (user: GroupUser) => user.punishments.reduce((acc, curr) => acc + curr.amount, 0)
