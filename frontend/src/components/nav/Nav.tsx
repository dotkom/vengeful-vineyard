...
export const Nav = ({ auth }: NavProps) => {
	...
	const isInAnyOWGroup =
		user?.groups.some((group) => group.ow_group_id !== null) ?? false;

	const prefetchWallOfShame = () =>
		queryClient.prefetchInfiniteQuery(leaderboardQuery());
	...
	const homeLocation =
		user && user.groups.length > 0
			? `/gruppe/${user.groups[0].name_short.toLowerCase()}`
			: "/";
	...
	const items: NavItem[] = [
		{
			label: "Hjem",
			url: homeLocation,
			isActivePredicate: (_, currentLocation) =>
				currentLocation.toLowerCase().startsWith("/gruppe/"),
		},
		{
			label: "Wall of Shame",
			url: "/wall-of-shame",
			isActivePredicate: (item, currentLocation) =>
				currentLocation.toLowerCase().startsWith(`${item.url}`),
							...
							<div className="flex flex-row justify-between md:justify-start w-full">
								...
								<div className="hidden md:ml-6 md:flex md:space-x-8">
									{items
										.filter(
											(item) =>
												!item.shouldShowPredicate ||
												item.shouldShowPredicate(item, location.pathname),
										)
										.map((item) => (
											<NavLink
												key={item.url}
												...
												isActive={item.isActivePredicate(
													item,
													location.pathname,
												)}
											/>
									...
									<Link key={item.url} to={item.url}>
										<Disclosure.Button
											...
											className={cn(
												...
												item.isActivePredicate(item, location.pathname)
