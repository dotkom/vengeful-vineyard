...
import { getAddPunishmentUrl, LEADERBOARD_URL } from "../../helpers/api";
...
export const Modal = forwardRef(({ setOpen }: ModalProps, ref) => {
  ...
  const { isLoading, error, data } = useQuery({
    ...
    queryFn: () =>
      axios.get(LEADERBOARD_URL).then((res: AxiosResponse<LeaderboardType>) => {
        setSelectedPerson(res.data.results[0]);
        res.data;
      }),
  });
