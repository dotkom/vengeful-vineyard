...
export const PunishmentItem = ({
	...
	let punishmentType = punishmentTypes
		? punishmentTypes[punishment.punishment_type_id]
		: undefined;
	...
	const { mutate: markPunishmentAsPaid } = useMutation(
		postPunishmentsPaidMutation(punishment.group_id!, [
			punishment.punishment_id,
		]),
	);

	const { mutate: markPunishmentAsUnpaid } = useMutation(
		postPunishmentsUnpaidMutation(punishment.group_id!, [
			punishment.punishment_id,
		]),
	);
	...
	const canMarkAsUnpaid = usePermission(
		...
		>
			{user && (
				<div className="flex flex-row gap-px items-center border-b dark:border-gray-800 border-gray-200 pb-3">
					...
					<span className="text-black text-base md:text-lg font-regular ml-auto">
						{groupName}
					</span>
				</div>
			)}
