...
export const SegmentEditorModal = ({
  open,
  setOpen,
  segments,
  onSave,
  ...
  const removeSegment = (index: number) => {
    if (editingSegments.length <= 2) return // Minimum 2 segments
    setEditingSegments((prev) => prev.filter((_, i) => i !== index))
  }

  const handleSave = () => {
    onSave(editingSegments)
    setOpen(false)
  }

  const handleClose = () => {
    setEditingSegments(segments) // Reset to original
    setOpen(false)
  }

  // Update the label when punishment type or amount changes
  const updateSegmentWithLabel = (index: number, punishmentTypeId: string | undefined, amount: number) => {
    ...
    const label = amount === 0 ? "Ingen!" : pt ? `${amount} ${pt.name}` : `${amount} straff`
    updateSegment(index, {
      punishmentTypeId,
      ...
      <Dialog as="div" className="relative z-50" onClose={handleClose}>
        ...
        <div className="fixed inset-0 z-10 overflow-y-auto">
          <div className="flex min-h-full items-center justify-center p-4">
            ...
            >
              <Dialog.Panel className="relative w-full max-w-md transform overflow-hidden rounded-2xl bg-white dark:bg-gray-800 p-6 shadow-2xl transition-all">
                ...
                <div className="max-h-[60vh] overflow-y-auto space-y-3 pr-1">
                  {editingSegments.map((segment, index) => (
                    <div
                      key={index}
                      className="flex items-center gap-2 rounded-lg border border-gray-200 dark:border-gray-600 bg-gray-50 dark:bg-gray-700 p-3"
                    >
                      {/* Color Picker */}
                      <div className="relative">
                        <input
                          ...
                          onChange={(e) =>
                            updateSegment(index, {
                              style: { ...segment.style, backgroundColor: e.target.value },
                            })
                          }
                      ...
                      {punishmentTypes.length > 0 ? (
                        ...
                        <input
                          type="text"
                          value={segment.option}
                          onChange={(e) => updateSegment(index, { option: e.target.value })}
                          className="flex-1 rounded-md border border-gray-300 dark:border-gray-500 bg-white dark:bg-gray-600 px-2 py-1 text-sm text-gray-900 dark:text-gray-100 focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500"
                          placeholder="Tekst"
                        />
                      )}
                      ...
                      {(segment.punishmentTypeId || punishmentTypes.length === 0) && segment.punishmentAmount !== 0 && (
                        <input
                          ...
                          min={1}
                          max={10}
                          value={segment.punishmentAmount}
                          onChange={(e) => {
                            const amount = parseInt(e.target.value) || 1
                            if (punishmentTypes.length > 0 && segment.punishmentTypeId) {
                              updateSegmentWithLabel(index, segment.punishmentTypeId, amount)
                            } else {
                              updateSegment(index, { punishmentAmount: amount })
                            }
                          }}
                          className="w-14 rounded-md border border-gray-300 dark:border-gray-500 bg-white dark:bg-gray-600 px-2 py-1 text-center text-sm text-gray-900 dark:text-gray-100 focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500"
                          title="Antall"
                        />
                      )}
                      ...
                      <button
                        ...
                        disabled={editingSegments.length <= 2}
                        className="rounded p-1 text-gray-400 hover:bg-red-50 dark:hover:bg-red-900/30 hover:text-red-500 disabled:opacity-30 disabled:hover:bg-transparent disabled:hover:text-gray-400"
                      >
                ...
                <div className="mt-3 flex gap-2">
                  <button
                    onClick={addSegment}
                    className="flex flex-1 items-center justify-center gap-1 rounded-lg border-2 border-dashed border-gray-300 dark:border-gray-600 py-2 text-sm text-gray-500 dark:text-gray-400 hover:border-indigo-400 hover:text-indigo-500"
                  >
                    <PlusIcon className="h-4 w-4" />
                    Straff
                  </button>
                  <button
                    onClick={addFreeSegment}
                    className="flex flex-1 items-center justify-center gap-1 rounded-lg border-2 border-dashed border-green-300 dark:border-green-600 py-2 text-sm text-green-500 dark:text-green-400 hover:border-green-400 hover:text-green-600"
                  >
                    <PlusIcon className="h-4 w-4" />
                    Ingen straff
                  </button>
                  ...
                  <div className="flex flex-wrap gap-1">
                    {PRESET_COLORS.slice(0, 5).map((color, colorIndex) => (
                      <button
                        key={color}
                        onClick={() => {
                          // Apply colors in rotation
                          setEditingSegments((prev) =>
                            prev.map((seg, i) => ({
                              ...seg,
                              style: {
                                ...seg.style,
                                backgroundColor: PRESET_COLORS[(i + colorIndex) % PRESET_COLORS.length],
                              },
                            }))
                          )
                        }}
