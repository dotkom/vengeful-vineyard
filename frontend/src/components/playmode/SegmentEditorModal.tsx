...
export const SegmentEditorModal = ({
  ...
  const updateSegment = (index: number, updates: Partial<WheelSegment>) => {
    setEditingSegments((prev) =>
      prev.map((seg, i) =>
        i === index ? { ...seg, ...updates, style: { ...seg.style, ...(updates.style || {}) } } : seg
      )
    )
  }
  ...
  const updateSegmentWithLabel = (index: number, punishmentTypeId: string | undefined, amount: number) => {
    ...
    updateSegment(index, {
      ...
      option: amount === 0 ? "Ingen!" : pt ? `${amount} ${pt.name}` : `${amount} straff`,
    })
  }
          ...
          {editingSegments.map((segment, index) => (
            ...
            >
              <input
                ...
                onChange={(e) => updateSegment(index, { style: { ...segment.style, backgroundColor: e.target.value } })}
              ...
              {punishmentTypes.length > 0 ? (
                <select
                  ...
                  onChange={(e) =>
                    updateSegmentWithLabel(
                      index,
                      e.target.value === "none" ? undefined : e.target.value,
                      e.target.value === "none" ? 0 : segment.punishmentAmount || 1
                    )
                  }
              ...
              {(segment.punishmentTypeId || punishmentTypes.length === 0) && segment.punishmentAmount !== 0 && (
                <input
                  ...
                  onChange={(e) =>
                    punishmentTypes.length > 0 && segment.punishmentTypeId
                      ? updateSegmentWithLabel(index, segment.punishmentTypeId, parseInt(e.target.value) || 1)
                      : updateSegment(index, { punishmentAmount: parseInt(e.target.value) || 1 })
                  }
              ...
              <button
                ...
                onClick={() =>
                  setEditingSegments((prev) =>
                    prev.map((seg, i) => ({
                      ...seg,
                      style: { ...seg.style, backgroundColor: PRESET_COLORS[(i + colorIndex) % PRESET_COLORS.length] },
                    }))
                  )
                }
