...
CREATE TABLE IF NOT EXISTS group_punishments (
	...
	group_id uuid NOT NULL references groups(group_id) ON DELETE CASCADE,
	user_id uuid NOT NULL references users(user_id) ON DELETE CASCADE,
	punishment_type_id uuid NOT NULL references punishment_types(punishment_type_id) ON DELETE CASCADE,  -- fix this at some point
...
CREATE TABLE IF NOT EXISTS paid_punishments_logs (
    paid_punishment_log_id uuid PRIMARY KEY DEFAULT uuid_generate_v4(),
	group_id uuid NOT NULL references groups(group_id) ON DELETE CASCADE,
	user_id uuid NOT NULL references users(user_id) ON DELETE CASCADE,
	value INTEGER NOT NULL,
	created_by uuid references users(user_id) ON DELETE CASCADE,
